...
if (isset($_POST['date'])) {
    ...
    $stmt = $pdo->prepare("SELECT heure FROM creneaux WHERE date = ? AND disponible = 1");
    $stmt->execute([$date]);
    $creneaux = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if ($creneaux) {
        foreach ($creneaux as $creneau) {
            echo "<option value='" . htmlspecialchars($creneau['heure']) . "'>" . htmlspecialchars($creneau['heure']) . "</option>";
        }
    } else {
